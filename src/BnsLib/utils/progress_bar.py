...
class progress_tracker():
    ...
    def eta(self):
        now = datetime.now()
        return(int(round(float((now - self.t_start).seconds) / self.steps_taken
                         * (self.num_of_steps - self.steps_taken))))
    ...
    def get_print_string(self):
        ...
        if int(curr_perc / 4) > 0:
            done_str = '=' * (int(curr_perc / 4) - 1) + '>'
            rest_str = '.' * (bar_len - int(curr_perc / 4))
            s = f'[{done_str}{rest_str}]'
        else:
            s = '[' + '.' * bar_len + ']'
        ...
        out_str = f'{curr_str}/{tot_str}: {s} {perc_str}% ETA: {eta}'
    ...
    def print_final(self, update=True):
        done_str = 25 * '='
        seconds_elapsed = (datetime.now() - self.t_start).seconds
        time_elapsed = timedelta(seconds=seconds_elapsed)
        final_str = (f'{self.steps_taken}/{self.num_of_steps}: [{done_str}] '
                     f'100% - Time elapsed: {time_elapsed}s')
        if update:
...
class mp_progress_tracker(progress_tracker):
    ...
    def eta(self):
        now = datetime.now()
        return(int(round(float((now - self.t_start).seconds)
                         / float(self.steps_taken.value)
                         * float(self.num_of_steps - self.steps_taken.value))))
    
    @property
    def percentage(self):
        return(int(100 * float(self.steps_taken.value)
                   / float(self.num_of_steps)))
    ...
    def print_final(self, update=True):
        ...
        seconds_elapsed = (datetime.now() - self.t_start).seconds
        time_elapsed = timedelta(seconds=seconds_elapsed)
        final_str = (f'{self.steps_taken.value}/{self.num_of_steps}: '
                     f'[{done_str}] 100% - Time elapsed: {time_elapsed}s')
        if update:
            clear_str = '\b \b' * self.last_string_length
