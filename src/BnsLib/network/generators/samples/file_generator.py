...
class FileGenerator(keras.utils.Sequence):
    ...
    def __getitem__(self, idx):
        ...
        if isinstance(self.file_handeler.input_shape, list):
            X = [np.zeros([batch_size] + list(shape))
                 for shape in self.file_handeler.input_shape]
        else:
            X = np.zeros([batch_size] + list(self.file_handeler.input_shape))
        
        if isinstance(self.file_handeler.output_shape, list):
            Y = [np.zeros([batch_size] + list(shape))
                 for shape in self.file_handeler.output_shape]
        else:
        ...
        if self.use_sample_weights:
            if isinstance(self.file_handeler.output_shape, list):
                W = [np.zeros(batch_size)
                     for _ in len(self.file_handeler.output_shape)]
            else:
                W = np.zeros(batch_size)
...
def format_batch(data, input_shape, output_shape):
    # data = [(input, output, sample_weights), (input, output, sample_weights), ...]  # noqa: E501
