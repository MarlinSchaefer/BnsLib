...
class TimeSeriesGenerator(Sequence):
    ...
    def __init__(self, ts, time_step=0.1, batch_size=32, dt=None,
        ...
        for t in ts:
            if isinstance(t, TimeSeries):
                ...
                if dt is None:
                    msg = 'If the provided data is not a '
                    msg += 'pycbc.types.TimeSeries a value dt must be '
                    msg += 'provided.'
        ...
        self.window_shifts = int(np.floor(float(len(self.ts[0])-self.window_size + self.stride) / self.stride))  # noqa: E501
    ...
    def __getitem__(self, index):
        ...
        if index == len(self) - 1:
            ...
            if len_data > len(self.ts[0]):
                max_stride -= int(np.floor(float(len(self.ts[0]) - len_data + self.batch_size) / self.stride))  # noqa: E501
        
        # Calculate the indices of the slices
        index_range = np.zeros((2, max_stride - min_stride), dtype=int)
        index_range[0] = np.arange(min_stride * self.stride, max_stride * self.stride, self.stride)  # noqa: E501
    ...
    def _gen_slices(self, index_range):
        ...
        for in_batch, idx in enumerate(index_range):
            low, up = idx
            if len(self.ts) > 1:
                detector_slices = [self.ts[det][low:up]
                                   for det in range(len(self.ts))]
            else:
