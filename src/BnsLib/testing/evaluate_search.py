...
def get_cluster_boundaries(triggers, boundarie_time=1.):
    ...
    idxs = idxs + 1
    idxs = np.concatenate([np.array([0]),
                          idxs,
                          np.array([len(trigger_times)])])
...
def get_event_list_from_triggers_mp(triggers, cluster_boundaries,
    ...
    if workers < 2:
        ...
        with mp.Pool(processes=workers) as pool:
            idxs = pool.starmap(get_idx, zip(sidxs,
                                             eidxs,
                                             repeat(sorted_triggers[1])))
