...
def generate_simple_injections(file_path, duration, seed=0,
    ...
    random_time_samples = int(round(float(signal_separation_interval) * float(srate)))  # noqa: E501
    signal_separation_samples = int(round(float(signal_separation) * float(srate)))  # noqa: E501
    time_samples = randint(signal_separation_samples - random_time_samples,
                           signal_separation_samples + random_time_samples,
                           size=size)
    time_samples = time_samples.cumsum()
    times = time_samples / float(srate)
    
    times = times[np.where(np.logical_and(times > padding,
                                          times < duration - padding))[0]]
    size = len(times)
