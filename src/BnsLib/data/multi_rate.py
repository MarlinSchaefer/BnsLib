...
def multi_rate_sample(ts, samples_per_part, sample_rates, reverse=False,
                      keep_end=True):
    ...
    for i, sr in enumerate(sample_rates):
        resampled_data, resampled_t = resample(ts.data,
                                               int(len(ts) / ts.sample_rate * sr),  # noqa: E501
                                               t=ts.sample_times)
...
def get_ideal_sample_rates(flow, fhigh, samples_per_part, mass1_min,
    ...
    # Adjust for the fact that the waveform is shifted by +/- time_variance
    time_variance = 2 * time_variance
